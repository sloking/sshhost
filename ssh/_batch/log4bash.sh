...
log() {
    ...
    if [ -z "$1" ];then
	echo -e "${log_color}[$(date +"%Y-%m-%d %H:%M:%S %Z")] ${LOG_DEFAULT_COLOR}";
    else
	echo -e "${log_color}[$(date +"%Y-%m-%d %H:%M:%S %Z")] ID ${log_pid} [${log_level}] ${log_text} ${LOG_DEFAULT_COLOR}";
    fi
    return 0;
}
...
log_success()   { log "$1" "$2" "SUCCESS" "${LOG_SUCCESS_COLOR}"; }
log_error()     { log "$1" "$2" "ERROR" "${LOG_ERROR_COLOR}"; }
#log_error()     { log "$1" "$2" "ERROR" "${LOG_ERROR_COLOR}"; log_speak "$1"; }
log_warning()   { log "$1" "$2" "WARNING" "${LOG_WARN_COLOR}"; }
log_debug()     { log "$1" "$2" "DEBUG" "${LOG_DEBUG_COLOR}"; }
